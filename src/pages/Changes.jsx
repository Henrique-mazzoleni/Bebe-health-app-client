...
function Changes() {
  ...
  const submitHandler = (e) => {
    e.preventDefault();
    axios
      .post(`http://localhost:5005/api/change/${childId}`, {
        dateAndTime,
        kind,
        consistency
      },
...
const getAllChanges = ()=>{axios.get(`http://localhost:5005/api/change/${childId}`,
{ headers: { Authorization: `Bearer ${storedToken}` } })
.then((response) => {
      ...
      <Form.Group className="mb-3" controlId="formGroupKind">
          ...
          <Form.Select aria-label="kind" onChange={kindHandler}>
            <option>Change Kind</option>
              ...
              <option className="dropDown" value="wet">Wet</option>
              <option className="dropDown" value="dirty">Dirty</option>
              <option className="dropDown" value="nothing">Nothing</option>
              <option className="dropDown" value="both">Both</option>
        ...
        <Form.Group className="mb-3" controlId="formGroupConsistency">
          ...
          <Form.Select aria-label="kind" onChange={consistencyHandler}>
            <option>Consistency</option>
              ...
              <option className="dropDown" value="liquid">Liquid</option>
              <option className="dropDown" value="runny">Runny</option>
              <option className="dropDown" value="soft">Soft</option>
              <option className="dropDown" value="Solid">Solid</option>
